...
public abstract class AbstractJdbc2Statement implements BaseStatement
{
    ...
    protected boolean executeReturning(String sql) throws SQLException
    {
        if (preparedQuery != null)
            throw new PSQLException(GT.tr("Can''t use query methods that take a query string on a PreparedStatement."),
                                    PSQLState.WRONG_OBJECT_TYPE);
        ...
        if (!executeWithFlags(sql, QueryExecutor.QUERY_BOTH_ROWS_AND_STATUS))
            throw new PSQLException(GT.tr("Query returning autogenerated keys didn't return anything."), PSQLState.NO_DATA);

        generatedKeys = result;
        firstUnclosedResult = generatedKeys;

        // Make update count available.
        result = generatedKeys.getNext();
